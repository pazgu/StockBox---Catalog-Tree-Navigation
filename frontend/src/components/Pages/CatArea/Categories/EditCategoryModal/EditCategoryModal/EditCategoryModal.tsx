...
const EditCategoryModal: React.FC<Props> = ({
  ...
  React.useEffect(() => {
    ...
    setPreviewImage(category.categoryImage.Image_url);
  }, [category]);
  ...
  const handleSave = async () => {
    ...
    const updated = {
      ...
      categoryImage: {
        Image_url: previewImage, // keep current preview image if not changed
        zoom: editZoom,
        offsetX: editOffset.x,
        offsetY: editOffset.y,
      },
    } as any;
                ...
                onWheel={(e) => {
                  ...
                  if (e.ctrlKey) {
                    ...
                    const nextZoom = Math.min(
                      4,
                      Math.max(1, +(editZoom + deltaZoom).toFixed(3)),
                    );

...
import { Trash, Pen, Lock, Heart } from "lucide-react";
...
const useCategoryManagement = (subcatName: string) => {
  ...
  const loadSubCategories = React.useCallback(async () => {
    try {
      setLoading(true);
      const data = await categoriesService.getSubCategories(subcatName);
      const mappedCategories: Category[] = data.map((cat: CategoryDTO) => ({
        id: cat._id,
...
const handleEditSave = async () => {
    ...
    try {
      const updatedCategory = await categoriesService.updateCategory(
        categoryToEdit.id,
        {
          ...
          categoryImage: categoryToEdit.image,
        }
      );
        ...
        prev.map((cat) =>
          cat.id === categoryToEdit.id
            ? {
                ...
                image: updatedCategory.categoryImage,
              }
            : cat
        )
      );
  ...
  const handleSave = async () => {
    if (newCatName && newCatImage) {
      try {
        const newCategory = await categoriesService.createSubCategory(
          subcatName,
          {
...
const SubCat: FC<SubCatProps> = () => {
  const navigate = useNavigate();
  const [showAddCatModal, setShowAddCatModal] = useState(false);

  const { subcatName } = useParams<{ subcatName: string }>();
  const {
    categories,
    setCategories,
    loading,
    ...
    handleEditSave,
    handleSave,
  } = useCategoryManagement(subcatName || "");

  const showDeleteModal = !!categoryToDelete;
  const showEditModal = !!categoryToEdit;
  const noCats = categories.length === 0 && !loading;
  ...
  if (loading) {
    return (
      <div className="p-4 font-system direction-rtl text-right flex items-center justify-center min-h-screen">
        <div className="text-xl text-slate-700">טוען...</div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="w-full flex justify-center flex-wrap gap-10 my-12">
        {noCats && <h3>לא נמצאו קטגוריות</h3>}

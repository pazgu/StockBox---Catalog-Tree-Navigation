...
const TOAST = {
  ...
  cannotDeleteLastSection: "לא ניתן למחוק את כל המקטעים. חייב להישאר לפחות מקטע אחד.",
} as const;
...
{section.type === "bullets" && (
  <BulletsSection
    ...
    onAdd={() => {
      const newBullet: BulletItem = { id: uid(), text: "נקודה חדשה" };
    ...
    onRemove={(id) => {
      ...
      setSections((prev) =>
        prev.map((s, i) =>
          i === sectionIndex && s.type === "bullets"
            ? { ...s, bullets: (s.bullets || []).filter((b) => b.id !== id) }
            : s
        )
      );
    }}
      ...
      setSections((prev) =>
        prev.map((s, i) =>
          i === sectionIndex && s.type === "bullets"
            ? {
                ...s,
                bullets: (s.bullets || []).map((b) => (b.id === id ? { ...b, text: value } : b)),
              }
            : s
        )
      );
  ...
  <>
    ...
    {(() => {
      ...
      return isEditing ? (
        ...
        <h2 className="text-[1.8rem] text-stockblue mb-4 font-bold">{section.title}</h2>
      );
    })()}

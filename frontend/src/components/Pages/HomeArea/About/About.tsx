...
const About: FC<AboutProps> = () => {
  ...
  const [sections, setSections] = useState<SectionType[]>([
    {
      ...
      content: 'היא מערכת וובי מרכזית שמאחדת את כל הידע של הצוות למקום אחד. המטרה שלנו היא להפוך מידע מפוזר למסודר, נגיש ובר-פעולה. כך שתוכלו למצוא במהירות נהלים, תיעוד, החלטות, קבצים ותוצרים חשובים, לשתף בקלות ולשמור על הנתונים הרצויים.'
    },
    {
      id: 'features-1',
      type: 'features',
  ...
  const handleSaveChanges = () => {
    const hasChanges = !isEqual(sections, originalData.sections) || !isEqual(editableImages, originalData.images);
    setIsEditing(false);
    if (hasChanges) {
            ...
            >
                  ...
                  <div className="relative bg-white/60 backdrop-blur-[20px] px-9 py-8 my-4 mb-6 border border-white/30 shadow-[0_2px_16px_rgba(13,48,91,0.04),0_1px_4px_rgba(13,48,91,0.02)] rounded-[20px] transition-all duration-300 ease-out hover:transform hover:-translate-y-0.5 hover:shadow-[0_4px_24px_rgba(13,48,91,0.06),0_2px_8px_rgba(13,48,91,0.03)] hover:border-stockblue/10 before:content-[''] before:absolute before:top-0 before:right-0 before:w-[3px] before:h-full before:bg-gradient-to-b before:from-stockblue before:to-stockblue/30 before:rounded-r-[2px] before:opacity-70">
                    {isEditing ? (
                      <textarea
                        ...
                        onChange={(e) => updateIntroContent(sectionIndex, e.target.value)}
              ...
              {section.type === 'features' && (
                <FeaturesSection
                  isEditing={isEditing}
                  title={section.title}
                  onChangeTitle={(title) => updateSectionTitle(sectionIndex, title)}
                  features={section.features || []}
                  onAdd={() => {
                    setSections(prev => prev.map((s, i) => 
                      i === sectionIndex && s.type === 'features'
                        ? { ...s, features: [...(s.features || []), { icon: "כוכב", title: "כותרת חדשה", description: "תיאור חדש" }] }
                        : s
                    ));
                  }}
                  onRemove={(itemIndex) => {
                    setSections(prev => prev.map((s, i) => 
                      i === sectionIndex && s.type === 'features'
                        ? { ...s, features: (s.features || []).filter((_, ii) => ii !== itemIndex) }
                        : s
                    ));
                  }}
                  ...
                  onReorder={(newFeatures) => {
                    setSections(prev => prev.map((s, i) => 
                      i === sectionIndex && s.type === 'features' ? { ...s, features: newFeatures } : s
                    ));
                  }}
                  ...
                  onAdd={() => {
                    setSections(prev => prev.map((s, i) => 
                      i === sectionIndex && s.type === 'vision'
                        ? { ...s, visionPoints: [...(s.visionPoints || []), "נקודה חדשה"] }
                        : s
                    ));
                  }}
                  onRemove={(itemIndex) => {
                    setSections(prev => prev.map((s, i) => 
                      i === sectionIndex && s.type === 'vision'
                        ? { ...s, visionPoints: (s.visionPoints || []).filter((_, ii) => ii !== itemIndex) }
                        : s
                    ));
                  }}
                  onChange={(itemIndex, value) => {
                    setSections(prev => prev.map((s, i) => 
                      i === sectionIndex && s.type === 'vision'
                        ? {
                            ...s,
                            visionPoints: (s.visionPoints || []).map((p, ii) =>
                              ii === itemIndex ? value : p
                            )
                          }
                        : s
                    ));

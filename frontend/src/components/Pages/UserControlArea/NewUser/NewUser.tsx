...
const userSchema = z.object({
  firstName: z
    ...
    .regex(
      /^[א-תa-zA-Z\s]+$/,
      "שם פרטי יכול להכיל רק אותיות עבריות או אנגליות"
    ),
  ...
  lastName: z
    ...
    .regex(
      /^[א-תa-zA-Z\s]+$/,
      "שם משפחה יכול להכיל רק אותיות עבריות או אנגליות"
    ),

  phoneNumber: z
    .string()
    .min(1, "מספר טלפון הוא שדה חובה")
    .regex(/^(\+972|0)([2-9]\d{7,8}|5[0-9]\d{7})$/, "מספר טלפון לא תקין. "),
  ...
  password: z
    ...
    .regex(
      /^(?=.*[A-Za-z])(?=.*\d)/,
      "סיסמא חייבת להכיל לפחות אות אחת ומספר אחד"
    ),
...
const NewUser: React.FC = () => {
  ...
  const wideFieldClasses = "flex flex-col mb-3 min-w-[300px] max-w-[350px]";

  return (
    <div className="flex justify-center items-start pt-[120px] w-full box-border fixed top-16">
      <motion.div
        className="bg-white scale-90 origin-top transform-gpu border border-gray-200 rounded-lg shadow-2xl px-6 py-4 h-[400px] w-[90%] flex flex-col relative **max-w-4xl**"
      ...
      >
        ...
        <form
          ...
          className="w-full flex flex-row rtl text-right flex-wrap gap-5 justify-center"
        >
          <div className={wideFieldClasses}>
            <label
              htmlFor="firstName"
            ...
            <div className="flex flex-col w-1/2">
              ...
              <div className="relative">
                <select
                  {...register("role")}
                  id="role"
                  className={`cursor-pointer py-3 px-4 pl-10 border rounded-lg text-base outline-none transition-all duration-200 rtl text-right bg-white min-h-6 leading-6 appearance-none ${
                    errors.role
                      ? "border-red-500 shadow-[0_0_0_3px_rgba(239,68,68,0.1)]"
                      : "border-gray-300 focus:border-[#0D305B] focus:shadow-[0_0_0_3px_rgba(13,48,91,0.1)]"
                  }`}
